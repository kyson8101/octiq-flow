...
export function WorkerExecutionEvidence({ execution }: { execution: WorkerExecution }) {
  return <div className="orch-execution" aria-label="Host execution evidence" data-execution={execution.state}>
    <dl>
      ...
      <dt>Last activity</dt><dd><EvidenceTime at={execution.lastActivityAt} /></dd>
      <dt>Last meaningful progress</dt><dd>{execution.lastProgress || "Not observed"}{execution.lastProgressAt ? <> · <EvidenceTime at={execution.lastProgressAt} /></> : null}</dd>
      <dt>Current operation</dt><dd>{execution.currentOperation || "None"}</dd>
    </dl>
