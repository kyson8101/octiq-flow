...
import { isWatching, lastSaid, noticeFor, owed, preview } from "./notify";
...
describe("noticeFor", () => {
  it("titles every notice after the chat, so the banner names the work", () => {
    const n = noticeFor({ kind: "done", conversationId: "c1", chatTitle: "Fix the top bar", detail: "All three tests pass." });
    expect(n.title).toBe("Fix the top bar");
    expect(n.body).toBe("All three tests pass.");
  });

  it("says the turn ended even when the agent left no words", () => {
    const n = noticeFor({ kind: "done", conversationId: "c1", chatTitle: "Fix the top bar", detail: "" });
    expect(n.body).toBe("Finished.");
  });

  it("marks a permission ask as needing you", () => {
    const n = noticeFor({ kind: "permission", conversationId: "c1", chatTitle: "Fix the top bar", detail: "Edit — web/src/App.tsx" });
    expect(n.body).toBe("Needs permission: Edit — web/src/App.tsx");
  });

  it("quotes the question the agent is blocked on", () => {
    const n = noticeFor({ kind: "question", conversationId: "c1", chatTitle: "Fix the top bar", detail: "Which theme?" });
    expect(n.body).toBe("Asked: Which theme?");
  });

  it("tags one notice per chat per kind, so a second replaces the first", () => {
    const a = noticeFor({ kind: "done", conversationId: "c1", chatTitle: "One", detail: "a" });
    const b = noticeFor({ kind: "done", conversationId: "c1", chatTitle: "One", detail: "b" });
    const other = noticeFor({ kind: "permission", conversationId: "c1", chatTitle: "One", detail: "a" });
    expect(a.tag).toBe(b.tag);
    expect(a.tag).not.toBe(other.tag);
  });

  it("names an untitled chat rather than showing an empty banner", () => {
    const n = noticeFor({ kind: "done", conversationId: "c1", chatTitle: "", detail: "hi" });
