...
// session key (`chat:<conversationId>`) and a
...
/** The conversation id inside a chat session key. */
export function splitChatKey(key: string): { conversationId: string } | null {
...
export function nameRow(row: MemoryRow, source: NameSource): RowName {
  ...
  if (row.kind === "chat") {
    ...
    if (held) return { name: held.title, where: projectName(held.projectId) };
    ...
    return { name: "Chat", where: "not on this device" };
  }
