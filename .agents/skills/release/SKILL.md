...
name: release
description: >-
  Project-local OctiqFlow release workflow. Invoke with `/release` or an
  explicit ship, release, or deploy request in OctiqFlow. Commits current-branch
  changes, pushes, runs Rust and web tests, builds the client and
  `octiq-server`, then asks before restarting via
  `~/.octiqflow/restart.request` because restart stops all live agent chats.
  Never branches, opens a PR, or builds a desktop app.
---
