...
Implementation is in `/Users/kyson/03-projects/octiq-flow-octiqos`, branch
`codex/octiqos-mission-control`, preserving its earlier changes. Work is not yet
committed. The canonical docspace update remains pending because filesystem and
Obsidian access returned EPERM; this document is the local implementation record.
